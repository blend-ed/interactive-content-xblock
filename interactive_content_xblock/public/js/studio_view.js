function StudioView(runtime, element) {
    ...
    // ── Test Grading: listen for submitInteraction calls from preview iframe ──

    window.addEventListener('message', function(event) {
        if (!event.data || event.data.type !== 'studio-test-grading') return;
    ...
    // ── Test Grading: build iframe preview for JS-controlled grading ──

    function buildPreviewIframe() {
        var htmlContent = $(element).find('textarea[name=xb_html_content]').val() || '';
        var cssContent = $(element).find('textarea[name=xb_css_content]').val() || '';
        var jsContent = $(element).find('textarea[name=xb_js_content]').val() || '';
        ...
        return '<!DOCTYPE html><html><head>' +
            ...
            'window.submitInteraction = function(data) {' +
                'window.parent.postMessage({ type: "studio-test-grading", payload: data }, "*");' +
            '};' +
            jsContent +
            '<\/script>' +
            '</body></html>';
    ...
    function updateTestInputs() {
        ...
        if (!autoGrade || mode === 'none') {
            // Show live preview iframe — author interacts with content and JS determines correctness
            var previewHtml = buildPreviewIframe();
            if (!previewHtml) {
                container.append(
                    '<span class="tip setting-help">Add HTML content above to preview grading.</span>'
                );
            } else {
                container.append(
                    '<div style="margin-bottom:8px;">' +
                      '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">' +
                        '<span class="tip setting-help" style="margin:0;">Interact with the preview below. When your JS calls submitInteraction(), the grading result will appear.</span>' +
                        '<a href="#" id="test-preview-reload" class="button" style="font-size:0.8em;white-space:nowrap;">Reload Preview</a>' +
                      '</div>' +
                      '<iframe id="test-preview-iframe" sandbox="allow-scripts" ' +
                        'style="width:100%;min-height:200px;border:1px solid #ccc;border-radius:4px;background:#fff;" ' +
                        '></iframe>' +
                    '</div>'
                );
                // Set srcdoc via DOM property to avoid HTML-attribute escaping issues
                $(element).find('#test-preview-iframe')[0].srcdoc = previewHtml;
            }
            // Hide "Run Test" button — interaction triggers grading automatically
            $(element).find('#test-grading-btn').hide();
        } else if (mode === 'single') {
            $(element).find('#test-grading-btn').show();
            container.append(
    ...
    // ── Test Grading: reload preview ──

    $(element).on('click', '#test-preview-reload', function(e) {
        e.preventDefault();
        updateTestInputs();
    });
