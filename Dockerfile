...
RUN mkdir -p /usr/local/src/interactive-xblock

# Set the created directory as a Docker volume
VOLUME ["/usr/local/src/interactive-xblock"]
...
RUN echo "pip install -r /usr/local/src/interactive-xblock/requirements.txt" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "pip install -e /usr/local/src/interactive-xblock" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "cd /usr/local/src/interactive-xblock && make compile_translations && cd /usr/local/src/xblock-sdk" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
