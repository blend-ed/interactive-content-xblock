...
This is an Open edX XBlock (`interactive-xblock`) that lets course authors create interactive HTML/CSS/JS content with learner interaction tracking and auto-grading. The XBlock is registered as `interactive_xblock` in the entry points.
...
pytest tests/test_interactive_xblock.py  # run specific test file
...
The XBlock is split into three mixins composed in `InteractiveXBlock` (xblocks.py):

- **`InteractiveXBlockModelMixin`** (`models.py`) — All XBlock fields (content fields with `Scope.content`, learner state with `Scope.user_state`), scoring logic (`has_score = True`), and staff detection.
- **`InteractiveXBlockViewMixin`** (`views.py`) — `student_view`, `studio_view`, JSON handlers (`save_interaction`, `studio_submit`), and response evaluation (`_evaluate_response`).
- **`InteractiveXBlock`** (`xblocks.py`) — Composes the mixins with `XBlock` and `CompletableXBlockMixin`. Defines workbench scenarios.

### Frontend

- `public/js/interactive_xblock.js` — Student-facing JS, initialized as `InteractiveXBlockView`
...
- `static/css/interactive_xblock.css` — Student view styles
...
- Coverage targets the `interactive_xblock` package
